...
import idbworker from 'workerize-loader!../core/workers/idb'; // eslint-disable-line import/no-webpack-loader-syntax
const idb = idbworker();
...
const Loader = ({ loaderInfo }) => {
	...
	useEffect(
		() => {
			...
			idb.addEventListener('message', (message) => {
				if (
			...
			fetchGraphQL(queries[itemKey], {})
				.then((result, error) => {
					if (error !== undefined) {
						...
						(async () => {
							await idb.init(dataKey);
							const y = await idb.putArray(result.data[dataKey], itemKey);
